...
import tma.ntt.demo.service.TokenService;
...
public class AuthHelper {
    ...
    public static TokenResponse getTokenFromAuthCode(String authCode, String tenantId) {
        ...
        try {
            return tokenService.getAccessTokenFromAuthCode(tenantId, getAppId(), getAppPassword(), "authorization_code", authCode, getRedirectUrl()).execute()
                    .body();
        } catch (IOException e) {
    ...
    public static TokenResponse ensureTokens(TokenResponse tokens, String tenantId) {
        ...
        if (now.getTime().before(tokens.getExpirationTime())) {
            ...
            try {
                return tokenService
                        .getAccessTokenFromRefreshToken(tenantId, getAppId(), getAppPassword(), "refresh_token", tokens.getRefreshToken(), getRedirectUrl())
                        .execute().body();
            } catch (IOException e) {
                TokenResponse error = new TokenResponse();
                error.setError("IOException");
